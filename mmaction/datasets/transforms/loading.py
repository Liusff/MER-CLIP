...
class PrepareCASME3Info:
    def __init__(self, format=".jpg", modality="RGB", prefix=""):
        self.format = format
    ...
    def __call__(self, results):
        ...
        #results["start_index"] = int(results["filename"].split("_")[-1])
        results["filename_tmpl"] = self.format
        #results["filename_tmpl"] = self.prefix + "{:0>4d}" + self.format
        return results
...
class UniformSample(BaseTransform):
    ...
    def _get_sample_clips(self, num_frames: int) -> np.ndarray:
        ...
        if num_frames < self.clip_len:
            frame_inds = np.arange(self.clip_len)
            if self.out_of_bound_opt == 'loop':
                frame_inds = np.mod(frame_inds, num_frames)
            elif self.out_of_bound_opt == 'repeat_last':
                safe_inds = frame_inds < num_frames
                unsafe_inds = 1 - safe_inds
                last_ind = np.max(safe_inds * frame_inds, axis=1)
                new_inds = (safe_inds * frame_inds + (unsafe_inds.T * last_ind).T)
                frame_inds = new_inds
...
@TRANSFORMS.register_module()
class RawFrameDecode2(BaseTransform):
    """Load and decode frames with given indices.
    ...
    def transform(self, results: dict) -> dict:
        ...
        cache = {}
        frames_path_list = glob.glob(osp.join(directory, "*"+filename_tmpl))
        for i, frame_idx in enumerate(results['frame_inds']):
            # Avoid loading duplicated frames
            if frame_idx in cache:
                imgs.append(cp.deepcopy(imgs[cache[frame_idx]]))
            ...
            frame_idx += offset
            if modality == 'RGB':
                filepath = frames_path_list[frame_idx]
                img_bytes = self.file_client.get(filepath)
                # Get frame with channel order RGB directly.
                cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                imgs.append(cur_frame)
...
@TRANSFORMS.register_module()
class RawFrame_noSample_Decode(BaseTransform):
    """Load and decode frames with given indices.
    ...
    def transform(self, results: dict) -> dict:
        ...
        cache = {}
        frames_path_list = glob.glob(os.path.join(directory, "*.jpg")) + glob.glob(os.path.join(directory, "*.png"))
        assert len(frames_path_list) > 0
        for i, frame_path in enumerate(frames_path_list):
            # Avoid loading duplicated frames
            if frame_path in cache:
                imgs.append(cp.deepcopy(imgs[cache[frame_path]]))
                continue
            else:
                cache[frame_path] = i

            if modality == 'RGB':
                img_bytes = self.file_client.get(frame_path)
                # Get frame with channel order RGB directly.
                cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                imgs.append(cur_frame)
            elif modality == 'Flow':
        ...
        while len(imgs) < self.num_frames:
            frame_path = frames_path_list[-1]
            imgs.append(cp.deepcopy(imgs[cache[frame_path]]))
        
        while len(imgs) > self.num_frames:
            del imgs[-1]
        
        assert len(imgs) == self.num_frames
