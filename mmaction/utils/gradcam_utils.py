...
class GradCAM:
    ...
    def _register_hooks(self, layer_name: str) -> None:
        ...
        prev_module = prev_module.backbone.transformer.resblocks[layer] #.backbone .vision_encoder
    ...
    def _calculate_localization_map(self,
        ...
        if use_labels > 0:
            ...
            use_labels = torch.tensor([use_labels]).cuda()
            labels = [use_labels]
            print(labels)
        ...
        if self.is_recognizer2d:
            ...
            L, tg, c = gradients.size()
            h_ = w_ = 14
            gradients = gradients[1:, :, :].view(h_, w_, tg, 1, c)
            activations = activations[1:, :, :].view(h_, w_, tg, 1, c)
            
            # target shape: [B, Tg, C', H', W']
            gradients = gradients.permute(3, 2, 4, 0, 1)
            activations = activations.permute(3, 2, 4, 0, 1)
    ...
    def _alpha_blending(self, localization_map: torch.Tensor,
                        ...
                        alpha: float,
                        colormap: int = cv2.COLORMAP_JET) -> torch.Tensor:
        ...
        #heatmap = cv2.applyColorMap(np.uint8(255 * localization_map), 2)
        ...
        #blended_imgs = blended_imgs / np.max(blended_imgs)
