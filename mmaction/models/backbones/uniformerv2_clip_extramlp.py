...
#from mmcv.utils.misc import make_tuple
...
def to_2tuple(value):
    if isinstance(value, (tuple, list)):
        assert len(value) == 2, "Input is already a tuple or list, but not of length 2"
        return value
    return (value, value)
...
class Transformer(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.norm((1 - weight) * cls_token + weight * residual.unsqueeze(0))
        x = x.view(L, N, T_down, C)[1:, :, :, :]
        x = x.permute(2, 0, 1, 3).flatten(0, 1)  # T * L-1, N, C
        feat = torch.cat((out, x), dim=0) # T * L, N, C
        #print("feat.shape: ", feat.shape)
        return out.squeeze(0), feat.permute(1, 0, 2)
...
class UniFormerV2_clip_extramlp(BaseModule):
    """UniFormerV2:
    ...
    def __init__(
        ...
        if output_dim > 0:
            ...
            self.proj = nn.Parameter(scale * torch.randn(n_dim, output_dim))
            self.ln_post1 = nn.LayerNorm(output_dim)
            nn.init.constant_(self.ln_post1.weight, 1.0)
            nn.init.constant_(self.ln_post1.bias, 0.0)
            '''
            self.extra_mlp = Mlp(
                in_features=output_dim, 
                hidden_features=int(output_dim*4.0), 
                act_layer=nn.GELU, drop=0.
            )
            nn.init.constant_(self.extra_mlp.fc2.weight, 0.)
            nn.init.constant_(self.extra_mlp.fc2.bias, 0.)
            '''
            self.to_cls_decoder  = nn.Linear(n_dim, cls_output_dim, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out, feat = self.transformer(x)
        #print(out.shape)
        if self.output_dim <= 0:
            return out
        else:
            if self.proj is not None:
                out_clip = self.ln_post1(out @ self.proj)
                out_cls = self.to_cls_decoder(feat)
                out_cls = self.ln_final(out_cls)
                return out_clip, out_cls
            else:
                return out
