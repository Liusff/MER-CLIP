...
class Transformer(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        residual = x.view(L, N, T_down, C)[0].mean(1)  # L, N, T, C -> N, C
        out = self.norm((1 - weight) * cls_token + weight * residual.unsqueeze(0))  # 1, N, C
        x = x.view(L, N, T_down, C)[1:, :, :, :] # H * W, N, T, C
        x = x.permute(2, 0, 1, 3).flatten(0, 1)  # T * (H * W), N, C
        feat = torch.cat((out, x), dim=0) # 1+ T * H * W, N, C
...
class UniFormerV2_extralinear(BaseModule):
    """UniFormerV2:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #print(out.shape)
        if self.cls_output_dim <= 0:
            return out
        else:
            if self.to_cls_decoder is not None:
                out_cls = self.ln_final(self.to_cls_decoder(feat))
                return out_cls
            else:
                return out
