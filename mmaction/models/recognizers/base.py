...
class BaseRecognizer(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 open_clip_projector: bool = False) -> None:
        ...
        if backbone_freeze:
            ...
            for name, p in self.backbone.named_parameters():
                if name.startswith('conv1') or name.startswith('class_embedding') or name.startswith('positional_embedding') or name.startswith('ln_pre'):
                    print(f"Freeze {name}")
                    p.requires_grad = False
                elif name.startswith('ln_post') and not open_clip_projector:
                    print(f"Freeze {name}")
                    p.requires_grad = False
                elif name.startswith('transformer'):
                    print(f"Freeze {name}")
                    p.requires_grad = False
                elif name.startswith('proj') and not open_clip_projector:
                    print(f"Freeze {name}")
                    p.requires_grad = False
                else:
    ...
    def forward(self, epoch: int = 0,
        ...
        if mode == 'predict':
            data_samples, _ = self.predict(inputs, data_samples, **kwargs)
            return data_samples
        elif mode == 'loss':
