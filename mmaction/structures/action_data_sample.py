...
class ActionDataSample(BaseDataElement):
    ...
    def set_pred_coarse_label(self, value: LABEL_TYPE) -> 'ActionDataSample':
        """Set ``pred_label``."""
        self.set_field(format_label(value), 'pred_coarse_label', dtype=torch.Tensor)
        return self

    def set_pred_coarse_score(self, value: SCORE_TYPE) -> 'ActionDataSample':
        """Set score of ``pred_label``."""
        score = format_score(value)
        self.set_field(score, 'pred_coarse_score')
        
        assert len(score) == 7, \
            f'The length of coarse score {len(score)} should be '\
            f'equal to the num_classes 7.'
