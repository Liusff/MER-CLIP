...
from modules.ATM import SimBlock, IntegrationBlock, DiffBlock
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, layer_id: int, attn_mask: torch.Tensor = None, dropout = 0.):
        super().__init__()
        ...
        self.control_point_c = AfterReconstruction(d_model)
    ...
    def forward(self, x: torch.Tensor, use_checkpoint=True):       
        x = self.control_point_c(x)  #[bt,d,h,w]
        ...
        if use_checkpoint:
            ...
            x = h + self.drop_path(attn_out)

        else:
            x = h + self.drop_path(self.attention(self.ln_1.float()(x)))
        ...
        if use_checkpoint: 
            ...
            x = h + self.drop_path(self.mlp(self.ln_2.float()(x)))
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, dropout=None, ):
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, i, attn_mask, dropout=dropout[i]) for i in range(layers)])
        self.grad_checkpointing = True

    def forward(self, x: torch.Tensor):
        for r in self.resblocks:
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint.checkpoint(r, x)
            else:
                x = r(x)
        return x
...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 joint=False,
                 tsm=None, T=8,dropout = 0., emb_dropout = 0.,
                 ):
        super().__init__()
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if vision_layers == 12:                                                                         
                ...
                start = 14                                                     
            make_ATM(self.visual, T,locations_list=[start])
...
def build_model(state_dict: dict, tsm=None,T=8,dropout=0., joint=False,emb_dropout=0.,pretrain=True):
    ...
    for key in ["input_resolution", "context_length", "vocab_size"]:
        if key in state_dict:
            del state_dict[key]
    if tsm == True or tsm in ["tsm", "tokenshift"]:
