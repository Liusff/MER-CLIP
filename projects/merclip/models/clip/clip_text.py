...
_MODELS = {
    ...
    "CLIP-ViT-B/16": "resources/ViCLIP-B_InternVid-FLT-10M.pth",
}
...
class CLIP_TEXT(nn.Module):
    ...
    def tokenize(self, texts, context_length=32, truncate=True):
        """
...
def clip_text_b16(
    ...
    checkpoint_num=0,
    pretrained="resources/ViCLIP-B_InternVid-FLT-10M.pth",
):
    ...
    print("text_projection shape: ", new_state_dict['text_projection'].shape)
    if context_length != new_state_dict["positional_embedding"].size(0):
        # assert context_length < new_state_dict["positional_embedding"].size(0), "Cannot increase context length."
        print(f"Resize positional embedding from {new_state_dict['positional_embedding'].size(0)} to {context_length}")
        if context_length < new_state_dict["positional_embedding"].size(0):
            new_state_dict["positional_embedding"] = new_state_dict["positional_embedding"][:context_length]
        else:
            new_state_dict["positional_embedding"] = F.pad(
                new_state_dict["positional_embedding"],
                (0, 0, 0, context_length - new_state_dict["positional_embedding"].size(0)),
                value=0,
            )

    message = model.load_state_dict(new_state_dict, strict=False)
    print(f"Load pretrained weights from {pretrained}: {message}")
    logger.info(f"Load pretrained weights from {pretrained}: {message}")
    return model.eval()
    #return model
