...
from .utils import load_state_dict as my_load_state_dict
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.clip_projector = AttentionPoolingBlock(
            ...
            drop=0., attn_drop=0., drop_path=head_drop_rate, 
            norm_layer=partial(nn.LayerNorm, eps=1e-5), out_dim=clip_embed_dim
        )
        self.to_cls_decoder  = nn.Linear(embed_dim, cls_output_dim, bias=False)
        self.fc_norm = norm_layer(cls_output_dim)
        self.clip_norm = norm_layer(clip_embed_dim)
    ...
    def forward_features(self, x):
        ...
        clip_x = self.clip_projector(x)

        x = self.fc_norm(x.mean(1))
        clip_x = self.clip_norm(clip_x)
    ...
    def forward_decoder_features(self, x):
        ...
        clip_x = self.clip_projector(x_lis[-2])  #倒数第二层

        x = self.fc_norm(self.to_cls_decoder(x))
        clip_x = self.clip_norm(clip_x)

        return clip_x, x
    
    def forward(self, x):
        x = self.forward_decoder_features(x)
        
        return x


def load_state_dict(model, checkpoint, model_keys="model|module", input_size=224, patch_size=16, num_frames=16, model_prefix=''):
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        org_num_frames = 16
        T = org_num_frames // model.patch_embed.tubelet_size
        P = int((pos_tokens.shape[1] // T)**0.5)
        C = pos_tokens.shape[2]
        new_P = input_size // patch_size[0]
        # B, L, C -> BT, H, W, C -> BT, C, H, W
    ...
    if num_frames != 16:
        ...
        T = org_num_frames // model.patch_embed.tubelet_size
        pos_tokens = model.pos_embed
        new_T = num_frames // model.patch_embed.tubelet_size
        P = int((pos_tokens.shape[1] // T)**0.5)
        C = pos_tokens.shape[2]
        pos_tokens = pos_tokens.reshape(-1, T, P, P, C)
    ...
    my_load_state_dict(
         model, checkpoint_model, prefix=model_prefix)
...
@register_model
def vit_base_patch16_224(pretrained=False, prefix=None, **kwargs):
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
        depth=12,
    ...
    if pretrained:
        ...
        if prefix:
            ...
            for k, v in checkpoint['state_dict'].items():
                if prefix in k:
                    new_checkpoint[k[len(prefix):]] = v
                    print(k)
            load_state_dict(model, new_checkpoint)
        else:
